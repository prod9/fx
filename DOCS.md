# FX Documentation

This file is no longer the canonical documentation. The content has been split into
per-topic specs under [`docs/spec/`](docs/spec/).

* [Philosophy](docs/spec/philosophy.md)
* [Configuration](docs/spec/configuration.md)
* [App Fragments](docs/spec/app-fragments.md)
* [Controllers](docs/spec/controllers.md)
* [Middlewares](docs/spec/middlewares.md)
* [Database](docs/spec/database.md)
* [Database Migrations](docs/spec/migrations.md)
* [Logging](docs/spec/logging.md)
* [Background Workers](docs/spec/workers.md)
* [Mailer](docs/spec/mailer.md)
* [Error Utilities](docs/spec/errors.md)
* [Testing](docs/spec/testing.md)
* [Utilities](docs/spec/utilities.md)
* [Releasing](docs/spec/releasing.md)

See also [`docs/README.md`](docs/README.md) for how `docs/{spec,decisions,notes}/` is
organized.
